...
class DbHelper {
  ...
  static insertUser(Database db, String firstName, String lastName,
    ...
    List<Map<String, Object?>> result = await db.rawQuery("select * from Users where mobile_number='$mobileNumber'");
    if(result.isEmpty){
      try {
        await db.execute(
  ...
  static checkUser(Database db,String username,String password) async{
    try {
      List<Map<String, Object?>> result = await db.rawQuery("select * from Users where mobile_number='$username' and password='$password'");

      if(result.isEmpty){
        List<Map<String, Object?>> result = await db.rawQuery("select * from Users where mobile_number='$username'");
        print(result);
        if(result.isEmpty){
        ...
        Fluttertoast.showToast(msg: "Welcome ${result[0]['first_name']} ${result[0]['last_name']}");
        return true;
      }
    } on Exception catch (e) {
